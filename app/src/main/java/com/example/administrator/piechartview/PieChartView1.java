...
public class PieChartView1 extends View {
    ...
    private List<PieChartView1.ItemType> itemTypeList, leftTypeList, rightTypeList;
    ...
    public PieChartView1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void drawTitle() {
        ...
        for (int i = 0; i < count; i++) {
            mPath.reset();
            PieChartView1.ItemType itemType = rightTypeList.get(i);
            ...
            if (textAlpha > 0) {
                ...
                //获取具体数字
                mCanvas.drawText(itemType.getNum(), centerPoint.x + (endPoint.x - centerPoint.x) / 2,
...
//                        centerPoint.y + (itemTextSize + textPadding) * 4 / 5, mPaint);
        ...
        for (int i = 0; i < count; i++) {
            mPath.reset();
            PieChartView1.ItemType itemType = leftTypeList.get(i);
            double angle = 2 * Math.PI * ((startRadius + itemType.radius / 2) / 360d);
            int x = (int) (width / 2 + radius * Math.cos(angle));
            int y = (int) (height / 2 + radius * Math.sin(angle));
            Point startPoint = new Point(x, y);
            ...
            if (textAlpha > 0) {
                ...
                mCanvas.drawText(itemType.getNum(), centerPoint.x + (endPoint.x - centerPoint.x) / 2,
                        centerPoint.y + (itemTextSize + textPadding) * 4 / 5, mPaint);
//                mCanvas.drawText(itemType.getPercent(), centerPoint.x + (endPoint.x - centerPoint.x) / 2,
//                        centerPoint.y + (itemTextSize + textPadding) * 4 / 5, mPaint);//获取百分比数字
            }
        }
    ...
    private void drawPie() {
        ...
        mPaint.setStyle(Paint.Style.FILL);
        int sum = 0;
        for (PieChartView1.ItemType itemType : itemTypeList) {
            sum += itemType.widget;
        }
        float a = 360f / sum;
        float startRadius = defaultStartAngle;
     ...
     * @param itemType
     */
    public void addItemType(PieChartView1.ItemType itemType) {
